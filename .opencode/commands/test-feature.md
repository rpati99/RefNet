---
description: Writes and runs tests for a specific RefNet feature. Pass the spec name as argument e.g. /test-feature 05-backend-connection
allowed-tools: Bash(vitest)
---
...
Invoke the **refnet-test-writer** subagent with the 
following context:
...
Wait for refnet-test-writer to fully complete and 
...
Once refnet-test-writer has finished, invoke the 
**refnet-test-runner** subagent with the following 
context:
...
- `frontend/` — existing React dashboard, if present
- Run command:
  ` vitest run tests/test_$ARGUMENTS.`
...
- If refnet-test-writer reports it could not write 
...
**Step 2 — Test Results**
- Mirror the refnet-test-runner's structured report
